...
<div class="js-item-product {% if slide_item %}js-item-slide swiper-slide{% else %}col{% if columns == 2 %}-6 col-md{% if show_filters %}-4{% else %}-3{% endif %}{% else %}-12 col-md-4{% endif %}{% endif %} item item-product{% if not product.display_price %} no-price{% endif %}" data-product-type="list" data-product-id="{{ product.id }}" data-store="product-item-{{ product.id }}">
    ...
    {% endif %}
        ...
        {% if settings.quick_shop and product.available and product.display_price and not show_whatsapp_cta %}
            ...
            <div class="item-actions mt-4">
                {% if product.variations %}
                    ...
                    <a data-toggle="#quickshop-modal" data-modal-url="modal-fullscreen-quickshop" class="js-quickshop-modal-open {% if slide_item %}js-quickshop-slide{% endif %} js-modal-open js-fullscreen-modal-open btn btn-success px-4" title="{{ 'Compra rápida de' | translate }} {{ product.name }}" aria-label="{{ 'Compra rápida de' | translate }} {{ product.name }}" >{{ 'Agregar al carrito' | translate }}</a>
                {% else %}
                    ...
                    <form class="js-product-form" method="post" action="{{ store.cart_url }}">
                        ...
                        <input type="submit" class="js-addtocart js-prod-submit-form btn btn-success {{ state }} px-4 mb-1" value="{{ texts[state] | translate }}" {% if state == 'nostock' %}disabled{% endif %} />
